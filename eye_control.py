...
Please see: https://github.com/kcmahip/opencv-eye-tracking/blob/master/eye_control.py
...
while 1:
	...
	if ret==True:
		...
		for (x,y,w,h) in detected: #similar to face detection
			...
			pupilFrame = cv2.equalizeHist(frame[int(y+(h*.25)):int((y+h)), int(x):int((x+w))]) #using histogram equalization of better image. 
